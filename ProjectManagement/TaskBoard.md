...
- [x] 基于现有 `LevelData` 配置教学关。
- [x] 实现基础敌人 AI 行动生成，不再固定使用第一个敌人技能。
- [x] 建立教程步骤数据结构。
- [x] 实现教程遮罩、高亮、提示文本和等待条件。
- [x] 教程步骤 1：说明时间点和区域是战斗核心。
- [x] 教程步骤 2：展示 6 个时间点。
- [x] 教程步骤 3：展示八卦区域修正。
- [x] 教程步骤 4：引导打开百科。
- [x] 教程步骤 5：引导选择技能。
- [x] 教程步骤 6：引导长按放置指令。
- [x] 教程步骤 7：引导检视详情。
- [x] 教程步骤 8：揭示怪物行动并解释优先级。
- [x] 教程步骤 9：点击开始结算。
- [x] 教程步骤 10：引导打开战斗记录。
- [x] 教程步骤 11：解释战斗记录面板。
- [x] 教程步骤 12：演示进入/离开区域的修正变化。
- [x] 教程步骤 13：胜利后展示成长界面。
- [x] 实现百科入口和至少两个条目集合：区域修正、状态效果。
- [ ] 教学关完整通关测试。
...
- [x] 接入玩家角色与敌人的图片素材
- [x] 按 GDD 修正教学关玩家/敌人数量、生命值与攻击力配置。
- [x] 按 `GDD_Split/06_UI_UX.md` 拆分上方敌方时间轴、下方我方时间轴和左侧战斗 UI 交互控件。
- [x] 将我方技能释放入口改为左侧“设置指令 -> 技能分类 -> 技能”，不再依赖点击我方头像 UI。
- [x] 修复旧头像占位节点空数据报错，并将新时间轴移出旧混合矩阵容器。
- [x] 移除旧我方头像 UI 与旧时间轴头像列的场景节点/运行时生成，并去掉“玩家准备中……”状态显示。
- [x] 修复旧 UI 清理后 Godot 运行态崩溃风险：保留空兼容锚点、删除旧头像子节点，并将 UI 初始化改为空安全/幂等连接。
- [x] 移除 `SetManagerState` 的旧调试状态标题写入，并统一敌我时间轴的行头、槽位和尾列宽度参数。
- [x] 修正敌我时间轴运行态视觉尺寸不一致：移除我方专属横幅，固定敌我时间轴宿主同宽居中，并将时间轴行高对齐槽位 prefab 实际高度。
- [x] 大幅重构 `MainUI.tscn` 静态布局：删除旧下方混合指令矩阵、旧头像列和 `GameTest` 调试入口，改为场景内显式敌方时间轴、我方时间轴、左侧战斗交互与设置菜单锚点。
- [x] 按新版 `GDD_Split/06_UI_UX.md` 将右上角齿轮设置菜单改为“百科 / 退出游戏”，百科右侧初始为空且详情区保留滚动。
- [x] 按准备阶段规则收紧左侧战斗交互按钮显隐：我方行动时显示“设置指令/检视详情”，等待结算时只显示“开始结算”。
- [ ] 接入战斗播片占位资源。
- [ ] 预留 UI 音效接口。
- [x] 检查百科滚动或分页。
- [ ] 修复三关 demo 阻塞性问题。
